...
/**
 * package us.asu.silv.clustering.datastructure;
...
import us.asu.silv.clustering.metric.EuclidianDistance;
import us.asu.silv.clustering.metric.MetricInterface;

public class DataNodeSimilarity1{
	DataNodeSimilarity1 parent= null;
	...
	ArrayList<DataNodeSimilarity1> childs = new ArrayList<DataNodeSimilarity1>();
	int level=0;
	double max[];
	String name[];
	public String index="";
	...
	int k=4;
	public DataNodeSimilarity1(DataNodeSimilarity1 parent){
		this.parent= parent;
	}
	
	public DataNodeSimilarity1(DataNodeSimilarity1 parent, int k,List<JSONObject> Resultset,MetricInterface metric, boolean isleaf){
	...
	public DataNodeSimilarity1(DataNodeSimilarity1 parent, int k,List<JSONObject> Resultset,MetricInterface metric, String features, String currentMetric){
		
		
			try {
				
				index=""+ Resultset.get(0).getJSONObject("Metadata").getInt("Index");
	...
	private String[] createMatrixDistance(List<JSONObject> resultset) {
		...
		for(int i=0;i<resultset.size();i++){
			...
			String simID;
			try {
				simID = Integer.toString(resultset.get(i).getJSONObject("Metadata").getInt("simid"));
				simID = simID + "^"+resultset.get(i).getJSONObject("Metadata").getInt("Index");
	...
	String getCentroid(List<JSONObject> data2){
		...
		String r= null;
		try {
			r = "Centroid " +data2.get(data2.size()/2).getJSONObject("Metadata").getInt("simid");
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} 
	...
	private String getData() {
		...
		for (int i = 0; i < this.data.size(); i++) {
			if(i<this.data.size()-1)
				d=d+"{\"name\":\""+this.data.get(i).getJSONObject("Metadata").getInt("simid")+"\",\"metric\":\""+this.data.get(i).getJSONArray("Data").getJSONObject(0).getString("metric")+"\"},";
				//COMMENT
...
public class DataSimilarityMongo implements DataSimilarityInterface{
	DataSimilarityMongo parent= null;
	List <ArrayList<JSONObject>> l = new ArrayList<ArrayList<JSONObject>>();
	List<String> timeSerieslabel=null;
	List<ArrayList<Double>> matrixDistance= new ArrayList<ArrayList<Double>>();
	
	List<JSONObject> data = null;
	ArrayList<DataSimilarityMongo> childs = new ArrayList<DataSimilarityMongo>();
	int level=0;
	double max[];
	String name[];
	...
	int k=4;
	public DataSimilarityMongo(DataSimilarityMongo parent){
		this.parent= parent;
	}
	
	public DataSimilarityMongo(DataSimilarityMongo parent, int k,List<JSONObject> Resultset,MetricInterface metric, boolean isleaf){
	...
	public DataSimilarityMongo(DataSimilarityMongo parent, int k,List<JSONObject> Resultset,MetricInterface metric, String features, String currentMetric){
		...
		try {
			this.metrica=Resultset.get(0).getJSONObject("Meta").getString("Metric");
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
...
//		System.out.println("added to: "+ this.name[idmin] );
	...
	private String[] createMatrixDistance(List<JSONObject> resultset) {
		...
		for(int i=0;i<resultset.size();i++){
			...
			//		System.out.println("i: "+i+" j: "+j+ " : "+(new EuclidianDistance()).pointDist1(this.timeSerieslabel, resultset.get(i),resultset.get(j)));
				//	System.out.println("a: " +resultset.get(i)+"\n b: "+resultset.get(j));
					this.matrixDistance.get(i).add( (new EuclidianDistance()).pointDist1(this.timeSerieslabel, resultset.get(i),resultset.get(j)));
				}
			}
		}
	...
	private String getData() {
		...
		try {
		for (int i = 0; i < this.data.size(); i++) {
			int Sim= this.data.get(i).getJSONObject("Meta").getInt("SimID");
			String metric = this.data.get(i).getJSONObject("Meta").getString("Metric");
			if(i<this.data.size()-1)
				d=d+"{\"name\":\""+Sim+"\",\"metric\":\""+metric+"\"},";
				//d=d+"{\"name\":\""+this.data.get(i).getJSONObject("Metadata").getInt("simid")+"\",\"metric\":\""+this.data.get(i).getJSONArray("Data").getJSONObject(0).getString("metric")+"\"},";
			/*	for(int j=0;j<this.data.get(i).getJSONArray("Data").length();j++){
				d=d+"{\"name\":\""+this.data.get(i).getJSONObject("Metadata").getInt("simid")+"\",\"metric\":\""+this.data.get(i).getJSONArray("Data").getJSONObject(j).getString("metric")+"\"},";
				}*/
