...
public class ModelClusterMongoEnergy implements ClusteringInterface{
	...
	ArrayList<MetricClusterMongoEnergy> lista =  new ArrayList<MetricClusterMongoEnergy>();
	...
	public ModelClusterMongoEnergy(String collName) {
...
//		System.out.println(listofobject.get(modellist.size()-1).size());
		...
		for(int i=0;i<modellist.size();i++){
			...
			if(listofobject.get(i).size()>1)
			lista.add(new MetricClusterMongoEnergy(k,listofobject.get(i),true));
		}
	...
	//				System.out.println(i+" : "+Resultset.getJSONObject(i));
		...
		for(int i=0;i<modellist.size();i++){
		//	System.out.println(" second modelcluster: "+modellist.get(i)+"   "+ i+"    "+listofobject.size());
			if(listofobject.get(i).size()>1)
			lista.add(new MetricClusterMongoEnergy(k,listofobject.get(i),true));
		}
