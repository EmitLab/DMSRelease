...
public class DataSimilarityOpentsdbMatlab implements DataSimilarityInterface{
	DataSimilarityOpentsdbMatlab parent= null;
...
//////shengyu modified
	...
	ArrayList<DataSimilarityOpentsdbMatlab> childs = new ArrayList<DataSimilarityOpentsdbMatlab>();
	int level=0;
	double max[];
	String name[];
	...
	int k=4;
	public DataSimilarityOpentsdbMatlab(DataSimilarityOpentsdbMatlab parent){
		this.parent= parent;
	}
	
	public DataSimilarityOpentsdbMatlab(DataSimilarityOpentsdbMatlab parent, int k,List<JSONObject> Resultset,MetricInterface metric, boolean isleaf){
	...
	public DataSimilarityOpentsdbMatlab(DataSimilarityOpentsdbMatlab parent, int k,List<JSONObject> Resultset,MetricInterface metric, String features, String currentMetric){
	...
	//Shengyu Added
		private String[] inputforMatlab(List<JSONObject> resultset){
			...
			for(int i=0;i<resultset.size();i++){
				...
				double points1[] = null;
				try {
					simID = ""+resultset.get(i).getJSONObject("Meta").getInt("simid");//Integer.toString(resultset.get(i).getJSONObject("Metadata").getInt("simid"));
					simID = simID + "^"+resultset.get(i).getJSONObject("Meta").getInt("Index");
					simID = simID +"^" +this.metrica; 
					if (!nameList.contains(simID))
						nameList.add(simID);
